...
// Get OT restaurants — extract slug from URL for direct page fallback
const OT_RESTAURANTS = Object.entries(BOOKING_MASTER)
  .filter(([_, v]) => v.platform === 'opentable' && v.url)
  .map(([name, info]) => {
    // Handle both /r/slug and /slug URL formats
    const rMatch = info.url.match(/opentable\.com\/r\/([^?/]+)/);
    const plainMatch = info.url.match(/opentable\.com\/([^?/]+)$/);
    const slug = rMatch ? rMatch[1] : (plainMatch ? plainMatch[1] : null);
    return { name, booking_url: info.url, slug };
  });

const withSlug = OT_RESTAURANTS.filter(r => r.slug).length;
console.log(`🍽️  Found ${OT_RESTAURANTS.length} OpenTable restaurants (${withSlug} with direct slug)`);
...
async function checkRestaurant(page, name, slug) {
  ...
  try {
    ...
    // Detect Access Denied / IP block
    const blocked = await page.evaluate(() => {
      const text = document.body?.innerText || '';
      return text.includes('Access Denied') || text.includes("don't have permission") || text.includes('403 Forbidden');
    });
    if (blocked) {
      return { tier: 'error', dinner_slots: 0, early: 'error', prime: 'error', late: 'error', sample_times: [], platform: 'opentable', checked_date: new Date().toISOString(), error: 'access_denied' };
    }
    ...
    // FALLBACK: If search card shows 0 slots but we have a slug, check the direct /r/ page
    // Search cards sometimes don't render time slots even when the restaurant has availability
    if (total === 0 && slug && !bestCard.noAvail) {
      try {
        ...
        const directResult = await page.evaluate(() => {
          ...
          const noAvail = text.includes('Notify me') || text.includes('no online availability') || text.includes('fully booked');
          return { blocked: false, dinnerTimes, noAvail };
        });

        if (!directResult.blocked && directResult.dinnerTimes.length > 0) {
          if (DEBUG) console.log(`    📌 Direct page fallback found ${directResult.dinnerTimes.length} slots for ${name}`);
          // Re-parse the direct page times
          parsedTimes.length = 0;
          early = 0; prime = 0; late = 0;
        ...
        if (DEBUG) console.log(`    ⚠️ Direct page fallback failed: ${e.message?.slice(0, 50)}`);
      }
    }
...
async function main() {
  ...
  let checked = 0, open = 0, limited = 0, booked = 0, unknown = 0, notOnOT = 0, blocked = 0;
  let consecutiveUnknowns = 0;
  let totalRequests = 0; // track across all phases for preemptive VPN rotation
  const PREEMPTIVE_VPN_EVERY = 150; // rotate VPN proactively every 150 requests

  const BATCH_SIZE = 500;
  const BATCH_PAUSE_MS = 5 * 60 * 1000; // 5 min pause between batches to avoid throttling

  for (const restaurant of toCheck) {
    // Batch cooldown: pause + rotate VPN every 500 restaurants
    if (checked > 0 && checked % BATCH_SIZE === 0) {
      fs.writeFileSync(OUTPUT_FILE, JSON.stringify(results, null, 2));
      console.log(`\n  ⏸️  Batch ${checked / BATCH_SIZE} complete — saving & pausing 5 min...`);
      console.log(`     Progress: 🟢 ${open}  🟡 ${limited}  🔴 ${booked}  🚫 ${blocked}  ❓ ${unknown}\n`);
      await browser.close();
      await rotateVPN();
    ...
    // Preemptive VPN rotation every N total requests to avoid getting flagged
    if (totalRequests > 0 && totalRequests % PREEMPTIVE_VPN_EVERY === 0) {
      console.log(`\n  🔄 Preemptive VPN rotation at ${totalRequests} total requests...\n`);
      await browser.close();
      await rotateVPN();
      await sleep(5000);
      browser = await launchBrowser();
    ...
    // Retry on error — rotate VPN + restart browser on access_denied
    if (result.tier === 'error') {
      if (result.error === 'access_denied') {
        console.log(`  🚫 Access Denied — rotating VPN + restarting browser...`);
        await browser.close();
        await rotateVPN();
        await sleep(5000);
        browser = await launchBrowser();
      ...
      totalRequests++;
      sessionCount++;
      // If still access_denied after VPN rotation, try one more time
    ...
    // VALIDATION: reject results that smell like Access Denied false positives
    if (result.matched_name === 'Access Denied' || result.error === 'access_denied') {
      result = { tier: 'error', dinner_slots: 0, early: 'error', prime: 'error', late: 'error', sample_times: [], platform: 'opentable', checked_date: new Date().toISOString(), error: 'access_denied_unresolved' };
    }

    // Don't store error results — keep previous data if we have it
    if (result.tier === 'error') {
      blocked++;
      if (existing[key] && existing[key].tier !== 'error') {
        console.log(`  🚫 [${checked + 1}/${toCheck.length}] ${restaurant.name}: BLOCKED — keeping previous data (${existing[key].tier})`);
      } else {
        results[key] = result;
        console.log(`  🚫 [${checked + 1}/${toCheck.length}] ${restaurant.name}: BLOCKED — no previous data`);
      }
      checked++;
      sessionCount++;
      consecutiveUnknowns++;
      ...
      if (consecutiveUnknowns >= 5) {
        console.log(`\n  ⚠️  ${consecutiveUnknowns} consecutive blocks — rotating VPN...\n`);
        await browser.close();
        await rotateVPN();
        await sleep(5000);
        browser = await launchBrowser();
  ...
  // Only recheck real results — skip error/blocked entries
  const recheckKeys = Object.entries(results)
    .filter(([_, v]) => (v.tier === 'booked' || v.tier === 'limited') && v.tier !== 'error' && v.error !== 'access_denied' && v.error !== 'access_denied_unresolved' && v.matched_name !== 'Access Denied')
    ...
    for (let i = 0; i < recheckKeys.length; i++) {
      ...
      for (const tc of checks) {
        try {
          ...
          // Detect Access Denied / blocking in Phase 2
          const blocked = await page.evaluate(() => {
            const text = document.body?.innerText || '';
            return text.includes('Access Denied') || text.includes('403 Forbidden') || text.includes("don't have permission") || text.length < 300;
          });
          if (blocked) {
            p2blocked = true;
            console.log(`    ⚠️  Phase 2 blocked at ${tc.label} — rotating VPN...`);
            await browser.close();
            await rotateVPN();
            await sleep(5000);
            browser = await launchBrowser();
            page = await setupPage(browser);
            sessionCount = 0;
            break; // skip remaining time checks for this restaurant
          }
      ...
      // If blocked in Phase 2, don't update this restaurant's data
      if (p2blocked) {
        console.log(`    ⏭️  Skipping ${r.name} — blocked, keeping Phase 1 data`);
        continue;
      }
  ...
  // Only check real booked results — skip error/blocked entries
  const stillBooked = Object.entries(results)
    .filter(([_, v]) => v.tier === 'booked' && v.error !== 'access_denied' && v.error !== 'access_denied_unresolved' && v.matched_name !== 'Access Denied')
    .map(([k]) => k);
    ...
    for (let i = 0; i < stillBooked.length; i++) {
      ...
      for (const offset of OFFSETS) {
        try {
          ...
          // Detect Access Denied / blocking in Phase 3
          const blocked = await page.evaluate(() => {
            const text = document.body?.innerText || '';
            return text.includes('Access Denied') || text.includes('403 Forbidden') || text.includes("don't have permission") || text.length < 300;
          });
          if (blocked) {
            p3blocked = true;
            console.log(`    ⚠️  Phase 3 blocked for ${name} at +${offset}d — rotating VPN...`);
            await browser.close();
            await rotateVPN();
            await sleep(5000);
            browser = await launchBrowser();
      ...
      // If blocked, don't mark as locked — leave it unknown for next run
      if (p3blocked) {
        console.log(`  ❓ [${i+1}/${stillBooked.length}] ${name}: blocked — skipping`);
      } else if (opensIn) {
  ...
  // Also merge into slim file for the frontend — skip error/blocked results
  let slim = {};
  try { slim = JSON.parse(fs.readFileSync(SLIM_FILE, 'utf8')); } catch {}
  for (const [key, val] of Object.entries(results)) {
    if (val.tier === 'error' || val.matched_name === 'Access Denied') continue; // don't pollute slim with blocked data
    slim[key] = {
      tier: val.tier,
  ...
  // Also merge into main tonight_availability.json if it exists — skip error/blocked
  let mainAvail = {};
  try { mainAvail = JSON.parse(fs.readFileSync(MAIN_OUTPUT, 'utf8')); } catch {}
  for (const [key, val] of Object.entries(results)) {
    if (val.tier === 'error' || val.matched_name === 'Access Denied') continue;
    mainAvail[key] = val;
  }
  fs.writeFileSync(MAIN_OUTPUT, JSON.stringify(mainAvail, null, 2));

  // Count final stats including blocked
  const finalBlocked = Object.values(results).filter(v => v.tier === 'error' || v.matched_name === 'Access Denied').length;
  const finalReal = Object.values(results).filter(v => v.tier !== 'error' && v.matched_name !== 'Access Denied').length;
  ...
  console.log(`   🚫 Blocked/Access Denied: ${blocked} (kept previous data where available)`);
  console.log(`   📊 Final dataset: ${finalReal} real results, ${finalBlocked} blocked/error`);
