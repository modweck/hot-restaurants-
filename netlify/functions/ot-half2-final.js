/**
 * ot-half2-final.js — OT availability check (half 2 of 2)
...
const TEMP_FILE = path.join(__dirname, 'tonight_availability_ot_half2.json');
...
// Half 2: second half
const OT_RESTAURANTS = ALL_OT.slice(Math.ceil(ALL_OT.length / 2));
console.log(`🍽️  Half 2: ${OT_RESTAURANTS.length} / ${ALL_OT.length} OpenTable restaurants`);
...
async function main() {
  ...
  console.log(`\n🔍 Half 2: Checking ${toCheck.length} restaurants for ${CHECK_DATE}, party of ${PARTY_SIZE}`);
