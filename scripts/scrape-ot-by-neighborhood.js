...
// NYC neighborhoods
// Manhattan neighborhoods only
const NEIGHBORHOODS = [
  'Tribeca', 'SoHo', 'West Village', 'East Village', 'Greenwich Village',
  'Chelsea', 'Flatiron', 'Gramercy', 'Murray Hill', 'Midtown East',
  'Midtown West', 'Hell\'s Kitchen', 'Upper East Side', 'Upper West Side',
  'Harlem', 'Washington Heights', 'Lower East Side', 'NoLita', 'NoHo',
  'Chinatown', 'Little Italy', 'Financial District', 'Battery Park',
  'Meatpacking District', 'Kips Bay', 'Koreatown', 'Theater District',
  'Times Square', 'Columbus Circle', 'Lincoln Center', 'Morningside Heights',
  'East Harlem', 'Yorkville', 'Alphabet City', 'Bowery', 'Two Bridges',
  'Hudson Yards', 'NoMad', 'Inwood', 'Roosevelt Island',
];
...
async function main() {
  ...
  for (const hood of NEIGHBORHOODS) {
    try {
      const url = 'https://www.opentable.com/s?term=' + encodeURIComponent(hood + ' NYC') + '&dateTime=2026-03-28T19%3A30%3A00&covers=2&metroId=8';
      ...
      for (const card of cards) {
        ...
        existing.push({ ...card, neighborhood: hood });
        newInHood++;
        totalNew++;
      }

      console.log(hood + ': ' + cards.length + ' results, ' + newInHood + ' new (total new: ' + totalNew + ')');

      // Save after each neighborhood
      fs.writeFileSync(OUTPUT, JSON.stringify(existing, null, 2));
    } catch (e) {
      console.log(hood + ': error — ' + e.message.substring(0, 50));
    }
