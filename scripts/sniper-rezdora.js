/**
 * sniper-rezdora.js — Full Hybrid Sniper with 2Captcha
 *
 * Same proven v3 script that booked Four Horsemen in 2.7s.
 *
 * RUN: node scripts/sniper-rezdora.js
 */
...
// uid 64692867
const AUTH_TOKEN = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9.eyJleHAiOjE3NzkyNDA1OTgsInVpZCI6NjQ3MzQ1NTgsImd0IjoiY29uc3VtZXIiLCJncyI6W10sImV4dHJhIjp7Imd1ZXN0X2lkIjoxOTM1MTEzMDV9fQ.AfvNXZlRZxZYAZwDy-niEGBQdLkk-27hTks1M-ZIGp6r5Y9juBNwFirNF5Yf35FmnaQIw-xxGD_Lw7d35rlIYXMWANTTTEYqe1qkHDN0rUQirEcX5SnWADmPZwOjIjh0oTj7zDVR2wObvbAwfN3o6jQoPoemFjlYlBhhj35Wyniws6ua';
...
async function main() {
  log('REZDORA SNIPER — Hybrid API + 2Captcha');
  log(`   Target: ${TARGET_DATE} — 6:00 PM+ only (no early slots)`);
  ...
  const browser = await puppeteer.launch({
    ...
    headless: false,
    userDataDir: `${homedir()}/.resy-sniper-profile-rezdora`,
  ...
  for (let t = 0; t < Math.max(captchaTokens.length, 1); t++) {
    ...
    if (book.success) {
      ...
      log(`   Rezdora — ${TARGET_DATE} at ${slot.time}`);
