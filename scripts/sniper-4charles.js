...
async function apiFind() {
  ...
  try {
    ...
    let pick = null;
    // Priority: 4:00 PM
    for (const s of slots) { if ((s.date?.start || '').includes('16:00')) { pick = s; break; } }
    // Fallback: first slot 4:00 PM or later
    if (!pick) {
      for (const s of slots) {
        const t = s.date?.start || '';
        const hm = t.match(/(\d{2}):(\d{2})/);
        if (hm && parseInt(hm[1]) * 60 + parseInt(hm[2]) >= 960) { pick = s; break; } // 4:00 PM = 960
      }
    }
...
async function apiBook(bookToken) {
  ...
  try {
    ...
    const resp = await fetch('https://api.resy.com/3/book', { method: 'POST', headers: h, body: `book_token=${encodeURIComponent(bookToken)}` });
    const data = await resp.json();
...
async function main() {
  ...
  log(`   Target: ${TARGET_DATE} — 4:00 PM (fallback any after 4pm, then anything)`);
