 ...
 *
 * Flow:
 *   1. Opens Chrome at drop-5min (for manual fallback)
 *   2. Pre-solves 2 captchas via 2Captcha 35s before drop
 *   3. API polls /4/find 500ms before drop (4 checks, 500ms apart)
 *   4. Slot found → /3/details with captcha_token → /3/book
 *   5. If captcha token rejected → try second token
 *   6. If both fail → browser fallback for manual booking
 *
...
async function apiFind() {
  ...
  try {
    ...
    let pick = null;
    // Priority: 5:00 PM, then first slot after 4:30 PM, then anything
    for (const s of slots) { if ((s.date?.start || '').includes('17:00')) { pick = s; break; } }
    if (!pick) {
      for (const s of slots) {
        ...
        if (hm) {
          ...
          if (mins >= 990) { pick = s; break; } // 4:30 PM = 16*60+30 = 990
        }
      }
    }
...
async function main() {
  ...
  log(`   Target: ${TARGET_DATE} — 5:00 PM (fallback 10:00 PM)`);
  ...
  // Wait until 5 min before drop to open browser
  ...
  if (solveAt > new Date()) {
    log(`Waiting until ${DROP_HOUR}:${String(DROP_MINUTE).padStart(2,'0')} - 35s to solve captchas...`);
  ...
  // ── PHASE 0: Pre-solve 2 captchas in parallel ──
  ...
  if (captchaTokens.length === 0) {
    log('Both captchas failed — will try without token + browser fallback');
  }
  ...
  if (!slot) {
    log('No slots found. Refreshing browser — try manually.');
    await page.goto(resyUrl, { waitUntil: 'networkidle2', timeout: 15000 }).catch(() => {});
    await new Promise(() => {});
  }

  // ── PHASE 2: Details + Book (try each captcha token) ──
  for (let t = 0; t < Math.max(captchaTokens.length, 1); t++) {
    const captchaToken = captchaTokens[t] || null;
    log(`PHASE 2: Details (captcha token ${t + 1}/${captchaTokens.length || 1})...`);
    ...
    if (!details.success) {
      ...
      if (t < captchaTokens.length - 1) {
        log('Trying next captcha token...');
        continue;
      }
