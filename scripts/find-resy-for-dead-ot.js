...
const OUTPUT_FILE = path.join(__dirname, '..', 'data', 'resy_replacements.json');
...
async function main() {
  const deadOt = [];
  for (const [name, check] of Object.entries(OT_CHECK.results || {})) {
    if (check.valid) continue;
    if (RESUME && previous.results && previous.results[name]) continue;
    const master = MASTER[name];
    if (!master) continue;
    deadOt.push({ name, lat: master.lat, lng: master.lng });
  }

  let toCheck = QUICK ? deadOt.slice(0, 30) : deadOt;

  console.log(`\n🔍 Resy Replacement Finder (TRIPLE VERIFIED)`);
  console.log(`📊 Dead OT links to check: ${deadOt.length + Object.keys(previous.results || {}).length}`);
