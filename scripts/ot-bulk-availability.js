// OT Bulk Availability - Tomorrow 2026-04-12
// Paginates through NYC metro search with date/time filter
// Fast: ~20 pages × 3s delay = ~1 min total
//
// HOW TO USE:
...
(async () => {
  ...
  const PARTY_SIZE = 2;
  const results = {};
  window.__OT_BULK_AVAIL = results;

  console.log('%c[OT Bulk Availability — ' + CHECK_DATE + ']', 'color: #00b894; font-weight: bold; font-size: 14px');
  ...
  while (page <= MAX_PAGES && consecutiveEmpty < 3) {
    try {
      ...
      // Extract restaurants + slots from the Apollo state
      // Each restaurant has: restaurantId, name, profileLink (slug), slots
      // Pattern: match restaurantId then find slots within same block
      let found = 0;
      ...
      while ((m = ridPat.exec(html)) !== null) {
        ...
        // Extract times from this restaurant's block
        const slots = [];
        ...
        // Only keep if we haven't seen this rid yet
  ...
  console.log('Download: (()=>{const d=JSON.stringify(window.__OT_BULK_AVAIL,null,2);const b=new Blob([d],{type:"application/json"});const x=document.createElement("a");x.href=URL.createObjectURL(b);x.download="ot_bulk_availability_2026-04-12.json";x.click();})()');
})();
