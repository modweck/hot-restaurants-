/**
 * sniper-monkey-bar.js — Monkey Bar Sniper (9am drop, 21-day window)
 *
...
const fs = require('fs');
const LOG_FILE = require('path').join(__dirname, '..', `sniper-monkey-bar-${TARGET_DATE}.log`);
...
async function main() {
  ...
  log(`   Target: ${TARGET_DATE} — 6:00 PM (fallback closest to 6pm)`);
  ...
  const browser = await puppeteer.launch({
    ...
    headless: false,
    userDataDir: `${homedir()}/.resy-sniper-profile-monkey-bar`,
  ...
  for (let i = 0; i < 8; i++) {
    ...
    let msg = result.error ? 'error ' + result.error : result.noEveningSlots ? `${result.slotCount} slots but none 6pm+` : 'no slots';
