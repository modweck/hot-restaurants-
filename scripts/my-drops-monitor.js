/**
 * my-drops-monitor.js — Custom drop monitor for specific restaurants only
 *
 * Tracks only the restaurants you've requested snapshots for.
 * Waits until the drop time, then fires all checks in parallel.
 *
 * RUN:
 *   node scripts/my-drops-monitor.js --time midnight
 *   node scripts/my-drops-monitor.js --time 10am
 *   node scripts/my-drops-monitor.js --time 11am
 *   node scripts/my-drops-monitor.js --time noon
 */
...
const LOG_FILE = path.join(LOG_DIR, `my-drops-${TODAY}.log`);
const DATA_FILE = path.join(LOG_DIR, `my-drops-${TODAY}.json`);

const RESY_API_KEY = 'VbWk7s3L4KiK5fzlO7JD3Q5EYolJI7n5';
const RESY_TOKENS = [
  'eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9.eyJleHAiOjE3Nzk4NDE0NjksInVpZCI6NjQ3MzQ1NTgsImd0IjoiY29uc3VtZXIiLCJncyI6W10sImxhbmciOiJlbi11cyIsImV4dHJhIjp7Imd1ZXN0X2lkIjoxOTM1MTEzMDV9fQ.AHSDXPVPOcjQaFkrB4VmiXILRUPZX3MTPUijIpSUaUkU23b_cwRaC5GIMjxIwOJOu54NRjtNehAtjbRhr_Sf2p5EAJ2uaF_UZ-gtLz7Jjk_XW9wVmeA91Ph18Auftxf3ASjtdv9_cudYaXOXEymzvPQqw_6hEpm_XuS9CUzXFu9t0I2x',
];
let tokenIdx = 0;
...
// ── MY DROPS — only these 7 restaurants ──
const MY_DROPS = {
  midnight: [
    ...
    { name: '69 Leonard Street', venue_id: null, slug: '69leonardstreet', window: 14 },                             // Apr 26
...
async function main() {
  ...
  const timesToCheck = timeArg === 'all' ? Object.keys(MY_DROPS) : [timeArg];

  log('═══════════════════════════════════════════════════');
  log(`MY DROPS MONITOR — ${timesToCheck.join(', ')}`);
  log(`Date: ${TODAY}`);
  log('═══════════════════════════════════════════════════\n');
  ...
  // Pre-resolve venue IDs
  const allRestaurants = timesToCheck.flatMap(t => MY_DROPS[t] || []);
  ...
  // Wait until drop time if --wait flag
  ...
  for (const time of timesToCheck) {
    const restaurants = MY_DROPS[time];
