/**
 * ot-check-all.js — Check ALL OT restaurants for tomorrow's availability.
 * Based on ot-limited-and-booked.js (proven VPN rotation + block detection).
 *
 * RUN:   node scripts/ot-check-all.js
 *        node scripts/ot-check-all.js --split 2 --half 1   (computer 1)
 *        node scripts/ot-check-all.js --split 2 --half 2   (computer 2)
 *
 * OPTIONS:
 *   --date 2026-04-10   Check a specific date (default: tomorrow)
 *   --party 2           Party size (default: 2)
 *   --quick             Only check first 15 (for testing)
 *   --batch 200         Stop after N restaurants
 *   --split 2 --half 1  Split work across machines
 *   --no-future         Skip future availability check
 *
 * OUTPUT: ot_availability_YYYY-MM-DD[_halfN].json (separate file, never overwrites live data)
 */
...
const NO_FUTURE = args.includes('--no-future') || true; // Always skip future for this script
const RETRY_BLOCKED = false;
...
const VERIFIED_OPEN_FILE = path.join(FUNCTIONS_DIR, 'ot_verified_open_2026-04-09.json');
const OUTPUT_FILE = SPLIT_TOTAL > 1
  ? path.join(FUNCTIONS_DIR, `ot_availability_${CHECK_DATE}_half${SPLIT_HALF}.json`)
  : path.join(FUNCTIONS_DIR, `ot_availability_${CHECK_DATE}.json`);
...
// (existing OT data not needed — this checks all fresh)

// Load previous output (for incremental resume)
let prevOutput = {};
try { prevOutput = JSON.parse(fs.readFileSync(OUTPUT_FILE, 'utf8')); } catch {}

// ── Build restaurant list: ALL OT restaurants, skip verified open ──
let verifiedOpen = {};
...
for (const [name, info] of Object.entries(BOOKING_MASTER)) {
  ...
  // Skip kosher (already verified separately)
  const cuisine = (info.cuisine || '').toLowerCase();
  if (cuisine.includes('kosher') || cuisine.includes('jewish')) continue;
  // Skip verified open
...
// Split mode
if (SPLIT_TOTAL > 1) {
  const chunkSize = Math.ceil(toRecheck.length / SPLIT_TOTAL);
  const start = (SPLIT_HALF - 1) * chunkSize;
  toRecheck.splice(0, start);
  toRecheck.splice(chunkSize);
  console.log(`🔀 Split mode: half ${SPLIT_HALF}/${SPLIT_TOTAL} (${toRecheck.length} restaurants)`);
}
...
async function main() {
  ...
  for (const restaurant of list) {
    ...
    const prevTier = prevOutput[key]?.tier || 'none';
    // Skip if already successfully checked this run
    if (prevOutput[key] && prevOutput[key].tier && prevOutput[key].tier !== 'error') { checked++; continue; }
