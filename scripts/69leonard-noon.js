...
const LOG_FILE = path.join(LOG_DIR, '69leonard-noon.log');
...
async function main() {
  log('69 LEONARD NOON MONITOR — checking May 13');

  const now = new Date();
  const drop = new Date(now);
  drop.setHours(12, 0, 0, 0);
  if (drop <= now) drop.setDate(drop.getDate() + 1);
  log(`Waiting ${Math.round((drop - now) / 60000)} minutes until noon...`);
  ...
  try {
    ...
    fs.writeFileSync(path.join(LOG_DIR, '69leonard-noon.json'), JSON.stringify({ name: '69 Leonard Street', date: '2026-05-13', slots: slots.length, times, checked_at: new Date().toISOString() }, null, 2));
  } catch (e) {
    log('FATAL: ' + e.message);
  }
}
